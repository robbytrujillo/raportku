...
<section class="content">
  <div class="container-fluid">
    <div class="row">
      <div class="col">
        <form action="{{ route('deskripsicapaian.update', $pembelajaran->id) }}" method="POST" id="form-update-deskripsi">
        @csrf
        @method('PUT')
          <div class="card">
            <div class="card-header">
              ...
              <div class="mt-3">
                  <button type="button" class="btn btn-warning btn-sm float-left"  data-id="{{ $pembelajaran->id }}"  id="capaian-tp-button">
                    <i class="fas fa-pencil-alt"></i>
                    Edit Capaian TP
                  </button>
                  @if ($nilai->count() >= 1 && $siswa->count() >= 1)
            ...
            <div class="card-body">
              @if ($nilai->count() < 1)
                Data masih kosong, tambahkan melalui Capaian TP terlebih dulu!
              @elseif ($siswa->count() < 1)
                Data Siswa masih kosong!
              @else
                <div class="table-responsive">
                  <table id="myTableDeskripsi" class="table table-sm table-hover mb-0">
                    <thead>
                      <tr class="bg-dark text-white header-table {{ Auth::user()->dark_mode == '1' ? 'bg-light' : '' }}">
                        ...
                        <th scope="col" class="mw-200">Deskripsi Capaian Tinggi</th>
                        <th scope="col" class="mw-200">Deskripsi Capaian Rendah</th>
                      </tr>
                    </thead>
                  </table>
                </div>
...
@section('js')
  ...
  @include('pages.nilaiakhir._scriptdeskripsi')
@endsection
